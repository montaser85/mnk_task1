            ...
            {#each btw_arrays_taken as btw_array, index}
              {#if btw_arrays_taken.length == index + 1}
                {#each btw_array as box, i}
                  {#if i == 0}
                    <rect
                      class="ChartBox {box_select!=null ? box_select == box? 'this-top-box-selected': 'this-top-box-not-selected' : 'top-pre-box'}"
              ...
              {console.log("Array Length: "+btw_arrays_taken.length)}
                ...
                {#each btw_array as box, i}
                  {#if i == 0}
                    <rect
                      class="ChartPreviousBox {box_select!=null ? box_select == box ? 'this-prev-top-box-selected': 'this-prev-top-box-not-selected':'prev-top-pre-box'}"
                      x={xValueUpdate(index)}
                      y={yValueUpdate(box, index)}
                      width={sttChartBoxWidth}
                      height={sttChartBoxHeight}
                  ...
                  {:else}
                    <rect
                      class="ChartPreviousBox {box_select!=null ? box_select == box ? 'this-prev-box-selected': 'this-prev-box-not-selected':'prev-pre-box'}"
                      x={xValueUpdate(index)}
                      y={yValueUpdate(box, index)}
                      width={sttChartBoxWidth}
                      height={sttChartBoxHeight}
